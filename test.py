...
## TODO: Add the rest of the maps once they are complete.
...
## TODO: Note that we have not yet completely dealt with error handling of inproper inputs. This will be taked care of when making the UI. Therefore, we currently have no test for inproper inputs.
...
def test_clean():
    assert clean('2FHALLWAY') == '2FHALLWAY'
    assert clean('FAQUARIUM') == '2FAQUARIUM'
    assert clean('ETMAINENTRANC') == 'EXTMAINENTRANCE'
    assert clean('THISPROJECTISAWESOME') == None
#Run tests
test_clean()
...
## TODO: Add for each map and double check the coastline nodes_expected value by counting by hand.
